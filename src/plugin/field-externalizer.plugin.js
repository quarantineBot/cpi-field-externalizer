...
(function () {
  ...
  function renderValues(ui, ctx, ids, model, params) {
    const { back, body, foot } = ui;
    body.innerHTML = ''; foot.innerHTML = '';
    body.appendChild(el('p', { textContent: `${ids.iflowName} — ${params.length} externalized parameter(s). Edit values or import a profile (changed rows highlighted). Apply saves the default values in place and reloads.` }));
    if (!params.length) {
      body.appendChild(el('p', { textContent: 'No externalized parameters found — externalize some fields first.' }, { color: '#777' }));
      const done = el('button', { textContent: 'Close' }, { padding: '8px 14px', cursor: 'pointer' }); done.onclick = () => back.remove(); foot.appendChild(done); return;
    }
    const orig = {}; params.forEach((p) => { orig[p.name] = p.value; });
    const table = el('table', {}, { width: '100%', borderCollapse: 'collapse' });
    table.innerHTML = '<thead><tr>' + ['Parameter', 'Value', 'Where', 'Type'].map((h) => `<th style="text-align:left;padding:6px;border-bottom:2px solid #ddd">${h}</th>`).join('') + '</tr></thead>';
    const tb = el('tbody');
    const cell = (h) => el('td', { innerHTML: h }, { padding: '6px', borderBottom: '1px solid #eee', verticalAlign: 'top' });
    const rows = params.map((p) => {
      const tr = el('tr');
      const inp = el('input', { type: 'text', value: p.value }, { width: '320px', font: '12px monospace' });
      ...
      const where = p.refs.map((r) => `${esc(r.name || '—')} <span style="color:#999">(${esc(r.kind)}: ${esc(r.detail)})</span>`).join('<br>') || '<span style="color:#aaa">—</span>';
      tr.append(c0, c1, cell(where), cell(`<span style="color:#777;font-size:11px">${esc(p.dataType.replace('xsd:', ''))}</span>`));
      tb.appendChild(tr);
      return { p, inp };
    });
    table.appendChild(tb); body.appendChild(table);
    ...
    foot.append(
      ...
      btn('Export JSON', {}, () => downloadText(`${ids.iflowName}.values.json`, pvm().serializeProfile(currentProfile(), 'json'))),
      btn('Export CSV', {}, () => downloadText(`${ids.iflowName}.values.csv`, pvm().serializeProfile(currentProfile(), 'csv'))),
      btn('Apply', { background: '#107e3e', color: '#fff', border: 'none' }, async () => {
